    ...
    public PartitionLoad getLoad() throws IOException{
        ...
        try(Admin admin=connection.getAdmin()){
            ...
            for(Map.Entry<ServerName,List<HRegionInfo>> entry:serverToRegionMap.entrySet()){
                ServerMetrics serverMetrics = clusterMetrics.getLiveServerMetrics().get(entry.getKey());
                if (serverMetrics == null) continue;
                Map<byte[], RegionMetrics> regionsLoad = serverMetrics.getRegionMetrics();
                for(HRegionInfo info:entry.getValue()){
                    RegionMetrics rl = regionsLoad.get(info.getRegionName());
                    if (rl == null) continue;
                    totalStoreFileSizeMB += (int) rl.getStoreFileSize().get(org.apache.hadoop.hbase.Size.Unit.MEGABYTE);
                    totalMemstoreSieMB += (int) rl.getMemStoreSize().get(org.apache.hadoop.hbase.Size.Unit.MEGABYTE);
                    storefileIndexSizeMB += (int) rl.getStoreFileIndexSize().get(org.apache.hadoop.hbase.Size.Unit.MEGABYTE);
                }
            }
        }
