...
public class BackupEndpointObserver extends BackupBaseRegionObserver implements CoprocessorService,Coprocessor {
    ...
    public void start(CoprocessorEnvironment e) throws IOException {
        try {
            ...
            String[] name = region.getTableDescriptor().getNameAsString().split(":");
            if (name.length == 2) {
                namespace = name[0];
                tableName = name[1];
            }
        ...
        try {
            ...
            try {
                ...
                for (StoreFile storeFile : postBulkLoadStoreFiles) {
                    if (!(storeFile instanceof HStoreFile)) continue;
                    HStoreFile hsf = (HStoreFile) storeFile;
                    byte[] val = hsf.getMetadataValue(HStoreFile.BULKLOAD_TASK_KEY);
