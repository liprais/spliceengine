...
public abstract class SpliceBaseOperation implements SpliceOperation, ScopeNamed, Externalizable{
    ...
    private void logExecutionStart(DataSetProcessor dsp) {
        ...
        if (!ignoreComentOptDisabled)
            stmt = activation.getLanguageConnectionContext().getOrigStmtTxt();
        else
            stmt = null;
         ...
         */
        String stmtForLogging = ignoreComentOptDisabled? ps.getSource() : (ps.getSourceTxt()==null? stmt:ps.getSourceTxt());
    ...
    private void logExecutionEnd() {
        ...
        ExecPreparedStatement ps = activation.getPreparedStatement();
        String stmtForLogging = ignoreComentOptDisabled? ps.getSource() : (ps.getSourceTxt()==null? stmt:ps.getSourceTxt());
        activation.getLanguageConnectionContext().logEndExecuting(uuid.toString(), stmtForLogging,
